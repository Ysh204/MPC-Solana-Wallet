...
export default function TipsPage() {
  ...
  return (
    <RequireAuth>
      <div className="max-w-5xl mx-auto" id="tips-history">
        <header className="mb-12">
          <h1 className="text-4xl font-extrabold tracking-tighter sm:text-5xl landing-gradient">
            Tip History
          </h1>
          <p className="text-[#a0a0b0] mt-2 font-medium">Track your community support and earnings</p>
        </header>

        <div className="flex gap-4 p-1.5 rounded-2xl bg-white/5 border border-white/10 w-fit mb-8 backdrop-blur-xl">
           <button
             onClick={() => setActiveTab("sent")}
             className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${activeTab === "sent" ? "bg-[#00f0ff]/10 text-[#00f0ff] border border-[#00f0ff]/20 shadow-[0_0_20px_rgba(0,240,255,0.05)]" : "text-[#a0a0b0] hover:text-white"}`}
           >
             Sent
           </button>
           <button
             onClick={() => setActiveTab("received")}
             className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${activeTab === "received" ? "bg-[#7000ff]/10 text-[#7000ff] border border-[#7000ff]/20 shadow-[0_0_20px_rgba(112,0,255,0.05)]" : "text-[#a0a0b0] hover:text-white"}`}
           >
             Received
           </button>
        </div>
            ...
            {tips[activeTab].length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {tips[activeTab].map((tip: any) => (
                  <div key={tip.id} className="tip-row group">
                    <div className="flex items-center gap-4 flex-1">
                       <div className={`w-12 h-12 rounded-full flex items-center justify-center border ${activeTab === "sent" ? "border-[#00f0ff]/20 bg-[#00f0ff]/5" : "border-[#7000ff]/20 bg-[#7000ff]/5"}`}>
                          <span className="text-xl">{activeTab === "sent" ? "📤" : "📥"}</span>
                       </div>
                       <div className="flex flex-col gap-1">
                          ...
                          <span className="text-[10px] uppercase font-mono tracking-widest text-[#a0a0b0] opacity-50">
                    ...
                    <div className="flex flex-col items-end gap-1">
                       <span className={`text-lg font-black ${activeTab === "sent" ? "text-white" : "text-[#00ff9d]"}`}>
